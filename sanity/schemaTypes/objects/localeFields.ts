...
export const localeBlockContent = defineType({
  ...
  fields: locales.map((locale) =>
    defineField({
      ...
      title: localeNames[locale],
      type: "array",
      // Paragraphs/headings/lists, plus the two editorial rhythm-breakers used
      // by the article body (see objects/articleBlocks.ts). Bodies that don't
      // need them simply never insert one.
      of: [{ type: "block" }, { type: "articleImage" }, { type: "pullQuote" }],
      fieldset: locale === defaultLocale ? undefined : "translations",
    }),
  ),
});
