 ...
 *
 * "No forms, no queues, just a conversation" (see the page copy right above
 * this form) — submitting builds a pre-filled message from the fields and
 * opens a WhatsApp chat via wa.me (lib/site.ts#whatsappLink) rather than
 * posting anywhere. No backend round-trip, so nothing here can fail server-side.
 *
...
export function ContactForm({ t }: { t: Dictionary["contact"]; lang: Locale }) {
  const f = t.form;
  const baseId = useId();
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (message.trim()) lines.push(`${f.message}: ${message.trim()}`);
