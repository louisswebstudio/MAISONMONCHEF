  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-linen sm:aspect-[16/9]">
        ...
        {total > 1 && (
          <>
            <GalleryArrow side={rtl ? "right" : "left"} label={labels.prev} onClick={() => go(-1)} />
            <GalleryArrow side={rtl ? "left" : "right"} label={labels.next} onClick={() => go(1)} />
          </>
        )}
        ...
        <span
          aria-live="polite"
          className="absolute bottom-[14px] end-[14px] inline-flex items-center bg-navy/85 px-[12px] py-[6px] text-[13px] font-medium leading-none tracking-[-0.2px] text-white backdrop-blur-sm"
        >
          {index + 1}
